 ...
 * Guides users through initial configuration when no .ralph-tui.yaml exists.
 ...
 * Config filename
 */
const CONFIG_FILENAME = '.ralph-tui.yaml';
...
/**
 * Save configuration to .ralph-tui.yaml
 */
  ...
  try {
    ...
    if (!options.force) {
      ...
      if (exists) {
        return {
          success: false,
          error: `Configuration file already exists: ${join(cwd, CONFIG_FILENAME)}. Use --force to overwrite.`,
        };
      }
    }
