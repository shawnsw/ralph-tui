...
<Steps>
  <Step title="Create an Epic">
    First, create an epic to group your tasks:

    ```bash
    bd create --title "Backend Refactor" --type epic
    # Created: beads-xyz
    ```
  </Step>

  <Step title="Create Tasks as Children of the Epic">
    Tasks must be **children** of the epic (using `--parent`) to appear when running with `--epic`:

    ```bash
    bd create --title "Database schema" --type task --parent beads-xyz
    # Created: beads-schema

    bd create --title "API endpoints" --type task --parent beads-xyz
    # Created: beads-api

    bd create --title "Frontend UI" --type task --parent beads-xyz
  ...
  <Step title="Add Dependencies Between Tasks">
