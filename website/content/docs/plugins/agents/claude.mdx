...
Or toggle in TUI:
- Press `t` to cycle through detail levels (minimal → moderate → full)
- Press `T` (Shift+T) to toggle the subagent tree panel visibility
...
2. Press `T` (Shift+T) to toggle the subagent tree panel visibility
