...
Skills are installed to your agent's skills directory. Each detected agent receives a copy of the bundled skills:
...
<Callout type="info">
You can manage skills manually with `ralph-tui skills list` and `ralph-tui skills install`.
Use `--local` to install to the project directory instead of the global directory.
</Callout>
...
# Create skills directory if needed (per agent)
mkdir -p ~/.claude/skills           # Claude Code
mkdir -p ~/.config/opencode/skills  # OpenCode
mkdir -p ~/.factory/skills          # Factory Droid
```
